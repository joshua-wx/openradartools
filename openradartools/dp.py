...
def apply_zdr_calibration_v2(radar, radar_dt, cal_df, in_zdr_name, out_zdr_name):
    #find zdr cal value
    error_msg = ''
    zdr_offset = 0
    if cal_df is None:
        ...
        cal_dates = pd.to_datetime(cal_df['date'].astype(str), format='%Y%m%d')
        radar_ts  = pd.Timestamp(radar_dt)
        cal_start = cal_dates.min()
        cal_end   = cal_dates.max()
        if radar_ts < cal_start or radar_ts > cal_end:
            print(f'WARNING: radar_dt {radar_dt.strftime("%Y-%m-%d")} is outside the ZDR calibration '
                  f'timeseries ({cal_start.date()} to {cal_end.date()}). Setting ZDR offset to 0.')
            error_msg = f'radar_dt outside calibration timeseries range ({cal_start.date()} to {cal_end.date()})'
        else:
            nearest_pos = (cal_dates - radar_ts).abs().argmin()
            zdr_offset  = float(cal_df['zdr'].iloc[nearest_pos])
    ...
    radar.fields[out_zdr_name]['calibration_description'] = 'Composite calibration using birdbath, bragg and Katsumata techniques'

    if len(error_msg) == 0:
        radar.fields[out_zdr_name]['calibration_warnings'] = 'None'
    else:
        radar.fields[out_zdr_name]['calibration_warnings'] = error_msg
    return radar
