...
def apply_zdr_calibration_v2(radar, radar_dt, cal_df, in_zdr_name, out_zdr_name):
    ...
    zdr_offset = 0
    if cal_df:
        zdr_offset = float(cal_df.loc[cal_df['date'] == radar_dt.strftime('%Y-%m-%d')]['zdr'].values[0])                 
    else:
    ...
    if radar_dt < datetime.strptime('20210101', '%Y%m%d'):
        ...
        radar.fields[out_zdr_name]['calibration_description'] = 'Calibration generated by S3CAR system ' \
                                                                'described in Louf and Protat (2023) ' \
                                                                'doi:10.1175/JTECH-D-22-0118.1'        
    
    if len(error_msg) == 0:
        radar.fields[out_zdr_name]['calibration_comment'] = 'Calibration derived from S3CAR calibration system'
    else:
